...
const UserLogin = () => {
  ...
  return (
    <div className='flex flex-col justify-between  p-4 h-screen'>
      <div>
        <div className="p-7">
          <form onSubmit={submitHandle}>
            ...
            <input className="bg-[#eeee] mb-7 rounded px-4 py-2 border w-full" autoComplete="username" required value={email} onChange={(e)=>{
              setEmail(e.target.value)
            }} type="email" placeholder="Your email" />
            <h3 className="mb-2 text-xl">
              Enter Your Password
            </h3>
            <input className="bg-[#eeee] mb-7 rounded px-4 py-2 border w-full" autoComplete="current-password" value={password} onChange={(e)=>{
      ...
      <Link to='/driver-sign' className='flex justify-center rounded-2xl text-xl mb-5 py-3  w-full bg-green-500'>
        Sign in as Driver
      </Link>
    </div>
  )
